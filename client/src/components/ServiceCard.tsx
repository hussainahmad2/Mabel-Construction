    ...
    >
      ...
      <CardContent className="relative p-8 text-center z-10">
        ...
        <div className="relative mb-8">
          <div className={`w-24 h-24 mx-auto bg-gradient-to-br from-primary via-primary/80 to-primary/60 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-xl ${
            isHovered ? 'rotate-12 scale-110 shadow-primary/25' : 'rotate-0 scale-100'
          }`}>
            <Icon className={`w-12 h-12 text-primary-foreground transition-all duration-500 ${
              isHovered ? 'scale-110' : 'scale-100'
            }`} />
          </div>
          
          {/* Enhanced 3D Shadow Effect */}
          <div className={`absolute inset-0 w-24 h-24 mx-auto bg-gradient-to-br from-primary/30 to-primary/10 rounded-2xl transform transition-all duration-500 -z-10 ${
            isHovered ? 'translate-x-4 translate-y-4 scale-110' : 'translate-x-2 translate-y-2 scale-100'
          }`} />
          
          {/* Floating Particles Effect */}
          <div className={`absolute inset-0 w-24 h-24 mx-auto transition-all duration-500 ${
            isHovered ? 'opacity-100' : 'opacity-0'
          }`}>
        ...
        <p className={`text-muted-foreground mb-6 leading-relaxed transition-all duration-300 ${
        ...
        <ul className="space-y-3 text-sm text-left mb-6">
          {features.map((feature, featureIndex) => (
            ...
            >
              <div className={`w-2 h-2 bg-primary rounded-full mr-4 flex-shrink-0 transition-all duration-300 ${
